...
class DiscountTest {
    ...
    fun `무비데이만 적용되면 10퍼센트 할인된다`() {
        ...
        val result = discount.discountAmountApply(amount = 10_000, screening = source)
    ...
    fun `오전 11시 정각과 오후 8시 정각은 시간 할인 조건에 해당하지 않는다`() {
        ...
        val morningResult = discount.discountAmountApply(amount = 10_000, screening = morning)
        val nightResult = discount.discountAmountApply(amount = 10_000, screening = night)
    ...
    @Test
    fun `시간 할인으로 음수가 되면 할인 단계에서 0원으로 보정된다`() {
        val source =
            screening(
                day = 9,
                startTime = LocalTime.of(10, 0),
        ...
        val result = discount.discountAmountApply(amount = 1_000, screening = source)

        assertThat(result).isEqualTo(0)
    }

    @Test
    fun `복합 할인 결과가 음수가 되면 0원으로 보정된다`() {
        val source =
            screening(
                day = 30,
                startTime = LocalTime.of(10, 0),
